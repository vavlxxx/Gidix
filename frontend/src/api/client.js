...
function readableStatus(message) {
  const known = {
    ...
    "Не удалось построить маршрут по дорожной сети.": "Не удалось построить маршрут.",
    "Не удалось привязать точку к дорожной сети.": "Не удалось привязать точку к дорожной сети.",
...
export const adminApi = {
  ...
  updateRouteGeometry: (id, data) => api(`/routes/${id}/geometry`, { method: "PUT", body: JSON.stringify(data) }),
  deleteRoute: (id) => api(`/routes/${id}`, { method: "DELETE" }),
  previewRoute: (data) => api("/routes/preview", { method: "POST", body: JSON.stringify(data) }),
  previewRoadRoute: (data) => api("/routes/preview-road", { method: "POST", body: JSON.stringify(data) }),
  buildRoutePlan: (data) => api("/routes/build-plan", { method: "POST", body: JSON.stringify(data) }),
